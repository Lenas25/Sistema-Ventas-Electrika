...
public class VendedorController implements ActionListener {
    ...
    private void listado() {
        tabla.setRowCount(0);
        view.jFechaIngreso.setText(null);
        view.jFechaSalida.setText(null);
    ...
    private void aceptarEditar() {
        Integer id = Integer.valueOf(view.txtId.getText());
        Vendedor v=dao.get(id);
        boolean bool = view.txtId.getText().isEmpty() || view.txtNombre.getText().isEmpty() || view.txtApellido.getText().isEmpty() || view.txtUsuario.getText().isEmpty();
        if (view.txtId.getText().isEmpty()) {
            ...
            String usuario =view.txtUsuario.getText();
            String contra;
            if (view.txtContra.getText()==null) {
                contra=v.getPassword();
            }else{
                contra = view.txtContra.getText();
            }
            String cargo = view.jComboCargo.getSelectedItem().toString();
            JOptionPane.showMessageDialog(null,
    ...
    private void cancelar() {
        ...
        view.txtId.setEnabled(false);
        view.btnBuscar.setEnabled(false);
        view.btnGuardarEntryExit.setEnabled(false);
        view.jFechaIngreso.setEnabled(false);
        view.jFechaSalida.setEnabled(false);
        view.btnGuardarEntryExit.setEnabled(false);
        listado();
    }
    ...
    private void registerEntryExit() {
        ...
        view.btnEditar.setEnabled(false);
        view.btnEliminar.setEnabled(false);
        view.btnAceptarAgregar.setEnabled(false);
        view.btnAceptarEditar.setEnabled(false);
        view.btnAceptarEliminar.setEnabled(false);
        if (view.tableUsers.getRowCount() == 0) {
            JOptionPane.showMessageDialog(null, "No hay datos para eliminar");
        } else if (fila >= 0) {
    ...
    private void guardarIngresoSalida() {
        if (view.jFechaIngreso.getText() != null ||view.jFechaSalida.getText() != null) {
            try {
                Integer id = Integer.valueOf(view.txtId.getText());
                String nombre = capitalizeFirstLetter(view.txtNombre.getText());
                String apellido = capitalizeFirstLetter(view.txtApellido.getText());
                ...
                listado();
                 ...
                 JOptionPane.showMessageDialog(null, "Ingrese las fechas respectivas");
            }
